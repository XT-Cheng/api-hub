...
export class CheckListResultMonitor {
    ...
    public static async getCurrentShift() {
        return ((await FetchRoute.fetch(CheckListResultMonitor.getShiftSql)) as any[]).map((row: any) => {
            return {
    ...
    private static async ifShiftChangeCheckListFinished(machine: {
        ...
        let checkListItems = ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getCheckListItemsSql
            , [CheckListResultMonitor.MachinesTBR, CheckListResultMonitor.ProcessTypeTBR]
            , [machine.Machine, CheckListResultMonitor.SHIFT_CHANGE_PROCESS_TYPE]))) as any[]).map((row: any) => {
        ...
        if (checkListItems.length == 0) {
            ...
            ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getCheckListDoneOfShiftChange
                ...
                format(CheckListResultMonitor.currentShift.shiftStart, dateFormat)]))) as any[]).map((row: any) => {
                    const result = {
    ...
    private static async ifChangeOverCheckListFinished(machine: {
        ...
        let checkListItems = ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getCheckListItemsSql
            , [CheckListResultMonitor.MachinesTBR, CheckListResultMonitor.ProcessTypeTBR]
            , [machine.Machine, CheckListResultMonitor.ARTICLE_CHANGE_PROCESS_TYPE]))) as any[]).map((row: any) => {
                return { HeaderId: row.PMDM_HEADER_ID, CheckListType: row.CHECKLIST_TYPE };
            });
        if (checkListItems.length == 0) {
            shouldChangeStatus = true;
            ...
            ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getCheckListDoneOfChangeOver
                , [CheckListResultMonitor.MachinesTBR, CheckListResultMonitor.HeaderIdTBR]
                , [machine.Machine, checkListItems[0].HeaderId]))) as any[]).map((row: any) => {
                    const result = {
                        HeaderId: row.PMDM_HEADER_ID,
                        Sequence: row.CHECKLIST_SEQUENCE,
                        Answer: row.CHECKLIST_ANSWER,
    ...
    private static async ShiftChangeCheck() {
        ...
        if (CheckListResultMonitor.currentShift.shiftNbr !== currentShift.shiftNbr
            ...
            let machines = ((await FetchRoute.fetch(CheckListResultMonitor.getMachinesSql)) as any[]).map((row: any) => {
                return <any>row.MACHINE;
            });

            machines = machines.map((machine) => (`'` + machine + `'`));
            // Select Machine which has Operation logged on
            machines = ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getMachineWithOperationSql
                , [CheckListResultMonitor.MachinesTBR]
                , [machines.join(',')]))) as any[]).map((row: any) => {
                    return row.MACHINE;
                });
            machines.forEach(async machine => {
                ...
                let checkListItems = ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getCheckListItemsSql
                    , [CheckListResultMonitor.MachinesTBR, CheckListResultMonitor.ProcessTypeTBR]
                    , [machine, CheckListResultMonitor.SHIFT_CHANGE_PROCESS_TYPE]))) as any[]).map((row: any) => {
                        return { HeaderId: row.PMDM_HEADER_ID, CheckListType: row.CHECKLIST_TYPE };
                    });
    ...
    private static async CheckListResultCheck() {
        ...
        let machines = ((await FetchRoute.fetch(replaceAll(CheckListResultMonitor.getMachineWithSpecificStatus
            , [CheckListResultMonitor.StatusTBR]
            , [[CheckListResultMonitor.SHIFT_CHANGE_STATUS, CheckListResultMonitor.ARTICLE_CHANGE_STATUS].join(',')]))) as any[]).map((row: any) => {
                return {
                    Machine: row.MACHINE,
                    Status: row.STATUS,
        ...
        machines.forEach(async machine => {
            ...
            if (shouldChangeStatus) {
                APIRoute.executeBAPI(new ChangeMachineStatus(machine.Machine, CheckListResultMonitor.PRODUCTION_STATUS, CheckListResultMonitor.USER_BADGE).dialogString())
            }
        });
    }
